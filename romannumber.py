class RomanNumber():
    ...
    def __gruposDeMil(self):
        ...
        for i in range(len(grupos)-1):
            ...
            if unidadesMayor < 4:
                grupoMenor[1] += unidadesMayor * 1000
                grupoMayor[1] -= unidadesMayor
            ...
            if numP > 0:
                for j in range(ix+1, ix+numP):
                    if grupoParentesis[j] != '':
                        raise ValueError('Símbolo entre parentesis de cierre')
        ...
        for letra in numRomano:  
            ...
            if letra in self.__valores:
                ...
                if ultimoCaracter == '':
                    ...
                    if ultimoCaracter in self.__valores5:
                        raise ValueError('No se pueden restar valores de 5')
                    
                    distancia = self.__simbolosOrdenados.index(letra) - self.__simbolosOrdenados.index(ultimoCaracter)
                    if distancia > 2:
